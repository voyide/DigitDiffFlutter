...
// Thread-safe Global LRU Cache using modern Rust OnceLock
fn get_cache() -> &'static Mutex<LruCache<String, BigUint>> {
    static CACHE: OnceLock<Mutex<LruCache<String, BigUint>>> = OnceLock::new();
    CACHE.get_or_init(|| Mutex::new(LruCache::new(NonZeroUsize::new(500000).unwrap())))
}
...
fn get_post_processed_val(n: &BigUint, cfg: &Config, engine: &Engine, l_ast: &Option<rhai::AST>, r1_ast: &Option<rhai::AST>, r2_ast: &Option<rhai::AST>) -> BigUint {
    ...
    let cached_solve = |num: &BigUint| -> BigUint {
        ...
        // Explicit type check to avoid E0282 Inference Error
        let cached_val: Option<&BigUint> = cache.get(&key);
        if let Some(res) = cached_val { 
            return res.clone(); 
        }
    ...
    if cfg.post_type == "NONE" {
        ...
        let r = idx / cfg.grid_c; 
        let c = idx % cfg.grid_c;
        let mut count = 0u32;
...
pub fn compile_gif_animation(mut cfg: Config, mode: String, frames_start: u32, frames_end: u32) -> Vec<u8> {
    let width = cfg.render_c as u16; 
    let height = cfg.render_r as u16;
    ...
    
    {
        ...
        for step in 0..total_frames {
            ...
            // Fix for E0277 & E0308: Passes ownership to Frame, explicitly handles transparency and palette.
